...
class OrderControllerTest {
    ...
    void placeOrder_missingUserId_returns400() throws Exception {
        Map<String, Object> body = Map.of(
                "items", List.of(Map.of("bookId", 10, "quantity", 1))
        );
        mockMvc.perform(post("/api/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
    ...
    void placeOrder_emptyItems_returns400() throws Exception {
        Map<String, Object> body = Map.of("userId", 1, "items", List.of());
        mockMvc.perform(post("/api/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isBadRequest())
    ...
    @Test
    @DisplayName("POST /api/orders returns 400 when service throws")
